...
type App struct {
	...
	AuthAPI *authAPI.AuthHandler // Keep existing auth handler for now
	// Add other API handlers
	CompAPI         *compAPI.CompatibilityAPI // Restore
	ContentAPI      *contentAPI.ContentAPI    // Restore
	DiagAPI         *diagAPI.DiagnosisAPI     // Restore
	PaymentAPI      *paymentAPI.PaymentAPI    // Restore
	SubscriptionAPI *subAPI.SubscriptionAPI   // Restore
}
...
func InitializeApp(ctx context.Context) (*App, error) {
	...
	app.AuthAPI, err = authAPI.NewAuthHandler(ctx, credentialsPath, userRepo) // Pass userRepo instead of AuthService
	if err != nil {
	...
	app.CompAPI = compAPI.NewCompatibilityAPI(app.CompatibilityService)      // Restore
	app.ContentAPI = contentAPI.NewContentAPI(app.ContentService)            // Restore
	app.DiagAPI = diagAPI.NewDiagnosisAPI(app.DiagnosisService)              // Restore
	app.PaymentAPI = paymentAPI.NewPaymentAPI(app.PaymentService)            // Restore
	app.SubscriptionAPI = subAPI.NewSubscriptionAPI(app.SubscriptionService) // Restore
